...
class LizzyDecoderLayer(nn.Module):
    def __init__(self, args: ModelArgs, layer_idx: int):
        ...
        self.self_attn = LizzyAttention(args, layer_idx=layer_idx)
        self.mlp = LizzyMLP(args)
        self.post_attn_norm = nn.RMSNorm(
            args.hidden_size, eps=args.rms_norm_eps
        )
        self.post_mlp_norm = nn.RMSNorm(
            args.hidden_size, eps=args.rms_norm_eps
        )
        self.args = args
    ...
    def __call__(
        ...
        r = self.post_attn_norm(self.self_attn(x, mask, cache))
        h = x + r
        r = self.post_mlp_norm(self.mlp(h))
        out = h + r
        return out
...
class LizzyModel(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.layers = [
            LizzyDecoderLayer(args=args, layer_idx=i)
